...
baseline.ipynb: Jupyther notebook containing my attempts.
  avg_diff: Finds the best matching root and its neigbours, then calculates an average. If the value is over the thrreshold, I classify it as "connection" (entailment or contradiction).
  threshold_diff: Finds the best matching root and its neigbours, then I check if the root and the neighbours are over a given threshold. If so, I classify it as "connection" (entailment or contradiction).
